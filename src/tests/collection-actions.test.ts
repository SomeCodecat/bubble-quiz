...
describe('Collection Actions', () => {
  ...
  describe('toggleCollectionLock', () => {
    it('should lock a collection and its questions', async () => {
      ...
      expect(mockDb.question.update).toHaveBeenCalledWith({
        where: { id: 'q1' },
        data: { isLocked: true, isPermanentlyPublic: false, ownerId: 'user-123' },
      });
    });
    ...
    it('should fail toggleLock if forbidden', async () => {
        mockDb.collection.findUnique.mockResolvedValue({ id: 'col-1', creatorId: 'other' });
        const result = await toggleCollectionLock('col-1');
  ...
  describe('addQuestionToCollection', () => {
    ...
    it('should fail if question already in collection', async () => {
        ...
        mockDb.question.findUnique.mockResolvedValue({ id: 'q1' });
        mockDb.questionCollection.findUnique.mockResolvedValue({ questionId: 'q1', collectionId: 'c1' });
  ...
  describe('deleteCollection', () => {
    it('should soft delete a collection', async () => {
      mockDb.collection.findUnique.mockResolvedValue({ id: 'col-1', creatorId: 'user-123' });
    ...
    it('should handle deep delete with multiple questions', async () => {
        ...
        mockDb.questionCollection.findMany.mockResolvedValue([
            { question: { id: 'q1', creatorId: 'user-123', collections: [{ collectionId: 'col-1' }] } },
            { question: { id: 'q2', creatorId: 'user-123', collections: [{ collectionId: 'col-1' }, { collectionId: 'other' }] } }
        ]);
  ...
  describe('renameCollection', () => {
    ...
    it('should fail if no session in renameCollection', async () => {
        ...
        await expect(renameCollection('col-1', 'New Name')).rejects.toThrow('Unauthorized');
    });
    ...
    it('should fail if not found', async () => {
        ...
        await expect(renameCollection('col-1', 'New Name')).rejects.toThrow('Collection not found');
    });
      ...
      expect(mockDb.question.findMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({
            AND: expect.arrayContaining([
                ...
                expect.objectContaining({ collections: { some: { collectionId: 'col-1' } } })
            ])
        })
      }));
    });
  ...
  describe('getAvailableCollections', () => {
    it('should return allowed collections', async () => {
      mockDb.collection.findMany.mockResolvedValue([{ id: 'c1', name: 'Public' }]);
  ...
  describe('getTrashedCollections', () => {
    it('should return trashed collections', async () => {
      mockDb.collection.findMany.mockResolvedValue([{ id: 'c1', name: 'Deleted' }]);
  ...
  it('should prevent deleting from locked collection if not owner', async () => {
    mockDb.collection.findUnique.mockResolvedValue({ id: 'c1', creatorId: 'other', isLocked: true });
  ...
  it('should prevent restoring forbidden collection', async () => {
    mockDb.collection.findUnique.mockResolvedValue({ id: 'c1', deletedById: 'other', isLocked: true });
